...
def dendrogram_plot(ax,linkage,partition,color_dict=None,lim_color=1.1,vertical_line=False,D=0,title=''):
    if color_dict is None:
        color_dict = make_color_dict(colors, partition, partition)
    ...
    #remove x labels
    if vertical_line:
        ax.axhline(y=lim_color, c='k',linestyle='--',linewidth=1.2,alpha=1)
    if D!=0:
        title = f'{title}, '+ rf'D={D:.3f}'
    ax.set_xticks([])
    ax.set_xticklabels([])
    ...
    ax.set_ylim(linkage[0][2],min(linkage[-1][2]+0.05,1))

def dendrogram_plot_one_color(ax,linkage,partition,color='k',lim_color=1.1,vertical_line=False,D=0,title=''):
    color_dict = {pid: color for pid in partition}
    dendrogram_plot(ax,linkage,partition,color_dict,lim_color,vertical_line,D,title)
    
    
def plot_entropy(ax,real_entropy,max_entropy,min_entropy,entropy_levels,balanceness=0,color=['teal','gray'],legend=True):   
    ax.plot(real_entropy,entropy_levels,'-',label='Real entropy',color=color[0])
    ax.plot(max_entropy,entropy_levels,'k--',label='Max entropy')
    ax.plot(min_entropy,entropy_levels,'k:',label='Min entropy')
    ax.fill_betweenx(entropy_levels, real_entropy, max_entropy, color=color[1], alpha=0.15)
    ax.fill_betweenx(entropy_levels, min_entropy, real_entropy, color=color[0], alpha=0.15)
...
def plot_lc_vs_mcmc(dendrogram,name):
    fig, axs = plt.subplots(1,3,figsize=(10, 6))  # set size
    axs = axs.flatten()
    #shuffle colors list
    color_dict = make_color_dict(colors, dendrogram.partition_lc, dendrogram.partition_mcmc)
    
    balanced_lc_cut = 1-dendrogram.list_D[np.argmax([x[0] for x in dendrogram.list_D])][1]
    dendrogram_plot(axs[0],dendrogram.linkage,dendrogram.partition_lc,color_dict,lim_color=balanced_lc_cut,vertical_line=True,D=dendrogram.D_lc_max,title='LinkClustering dendrogram')
    dendrogram_plot(axs[1],dendrogram.linkage,dendrogram.partition_mcmc,color_dict,D=dendrogram.D_mcmc_max,title='AdaptiveCut dendrogram')
    plot_entropy(axs[2],dendrogram.real_entropy,dendrogram.max_entropy,dendrogram.min_entropy,dendrogram.entropy_levels,balanceness=dendrogram.balanceness)
    
    plt.tight_layout()
    plt.savefig(f'figures/{name}__.png',dpi=300)
...
def adj_matrix_color(adj_matrix,partition):
    edge_partition = dendrogram_balanced.get_partition_edges(partition)
    edge_partition = {edge:partition for partition,edges in edge_partition.items() for edge in edges}
    edge_colors = {edge:color_dict[partition] for edge,partition in edge_partition.items()}
    adjacency_matrix_color = np.zeros((adj_matrix.shape[0],adj_matrix.shape[1],4))
    for i in range(adj_matrix.shape[0]):
        for j in range(i,adj_matrix.shape[1]):
            if adj_matrix[i,j] == 1:
                adjacency_matrix_color[i,j] = mcolors.to_rgba(edge_colors[(i,j)])
                adjacency_matrix_color[j,i] = mcolors.to_rgba(edge_colors[(i,j)])
            else:
                adjacency_matrix_color[i,j] = [1,1,1,0]
    return adjacency_matrix_color
